/*
  Bundle: t002_mixed_authenticity
  Source: MLM Analysis.sas (daily-logs Authenticity models A & B), ZvanAllen/Creativity-intervention

  Runs the author's multilevel models of Daily_Authenticity on the reshaped
  daily data: Model A (unconditional, random intercept) and Model B (random
  intercept + slope for Time, type=un). The upstream PROC IMPORT of the local
  WBC V8a .sav is replaced by a synthetic wbcMasterDaily with the exact columns
  the reshape reads; the reshape and both PROC MIXED calls are the author's own.
*/
...
proc mixed data= wbc_daily method=reml noclprint covtest;
	class ID;
